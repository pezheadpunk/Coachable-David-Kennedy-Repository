...
'''
Use list comprehension to create a list of the squares of all odd numbers from
1 to 20.
'''
...
def multiplication_table(rows: int, cols: int):
    '''
    Write a function multiplication_table(rows: int, cols: int) that generates a
    2D list representing a multiplication table of size rows x cols.
    '''
    return [[row * col for col in range(cols)] for row in range(rows)]

def merge_sorted_lists(list_1, list_2):
    '''
    Write a function merge_sorted_lists that takes two sorted lists and merges
    them into a single sorted list without using built in sorting functions.
    '''
    i, j = 0, 0
...
def first_occurance(target, nums):
    '''
    Write a function first_occurance that returns the index of the first
    occurrence of target in nums. If the target is not found, return -1.
    '''
...
def find_missing_number(nums_list):
    '''
    Write a fucntion find_missing_number that takes a list containing n distinct
